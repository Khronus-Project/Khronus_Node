...
def create_alert(_request_content):
    ...
    alert = {"id":_request_content["alertID"],'parent':_request_content['requestID'],'correlative':_request_content['alert_correlative'],'timestamp':datetime.fromtimestamp(_request_content["timestamp"]),}
    event = {"type_id":crud_event_type,"time":datetime.utcnow(),"task_type_id":task_type, "khron_request_id":khron_request_id, "alert_id":_request_content["alertID"]}
    add_alert(alert)
    add_event(event)
    return True
